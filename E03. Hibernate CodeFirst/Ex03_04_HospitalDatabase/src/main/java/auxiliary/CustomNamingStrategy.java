...
public class CustomNamingStrategy extends PhysicalNamingStrategyStandardImpl {
    ...
    public Identifier toPhysicalTableName(Identifier name, JdbcEnvironment context) {
             return super.toPhysicalTableName(toSnakeCaseTableName(name), context);
    }
    ...
    private Identifier toSnakeCaseTableName(Identifier id) {
        if (id == null)
            return id;

        String name = id.getText();
        String snakeName = name.replaceAll("([a-z]+)([A-Z]+)", "$1\\_$2").toLowerCase() + "s";
        if (!snakeName.equals(name))
            return new Identifier(snakeName, id.isQuoted());
        else
            return id;
